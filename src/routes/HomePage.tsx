...
import { generatePlaceholder, fileSystemModule, ModuleContext, FILE_SYSTEM_MODULE_ID } from "../store/module";
...
const HomePage: React.FC = () => {
  ...
  const { data, isError } = useQuery({
    ...
    placeholderData: [...Array(10)].map(generatePlaceholder),
    queryKey: ["modules"],
    queryFn: async () => {
        ...
        <List sx={{ bgcolor: "background.paper" }}>
          {data?.map((module) => (
            <ModuleListItem
              key={module.id}
              module={module}
              onClick={module.id === FILE_SYSTEM_MODULE_ID ? handleLocalClick : undefined}
            />
          ))}
        </List>
      </Container>
